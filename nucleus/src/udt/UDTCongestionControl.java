...
public class UDTCongestionControl implements CongestionControl {
	...
	public void setAckInterval(long ackInterval) {
		...
		if (session.getSocket() != null
				&& session.getSocket().getReceiver() != null) {
	...
	private double computeNumOfIncreasingPacket() {
		// difference between link capacity and sending speed, in packets per
		// second
	...
	public void onLoss(List<Integer> lossInfo) {
		...
		long currentMaxSequenceNumber = session.getSocket().getSender()
		...
		if (firstBiggestlossSeqNo > lastDecreaseSeqNo) {
			...
			decCount = 1;
			/*
			 * - compute DecRandom to a random (average distribution) number
			 * between 1 and AvgNAKNum
			 */
